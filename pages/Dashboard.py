...
if global_inflation_df.empty:
    proxy_infl_df = df[df["indicator"] == "inflation"] if not df.empty else pd.DataFrame()
    if proxy_infl_df.empty and countries:
...
if global_unemployment_df.empty:
    proxy_unemp_df = df[df["indicator"] == "unemployment"] if not df.empty else pd.DataFrame()
    if proxy_unemp_df.empty and countries:
    ...
    for config in global_configs:
        ...
        with global_chart_cols[g_col_idx % 2]:
            ...
            if st.button(f"🔍 Expand {config['btn']}", key=f"expand_btn_{config['ind']}_global", use_container_width=True):
                open_expanded_chart(fig, hist_df, predictions_df[predictions_df['indicator'] == config["ind"]], config["ind"])
